...
export default function AITutorJourney({ onPractice }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Chủ đề ưu tiên không nằm trong bảng đánh giá: đọc lại từ lộ trình đã lưu,
        // nếu không thì lần "Cập nhật lộ trình" sẽ mất chủ đề người học đã nhập.
        const savedTopics = roadmapData?.topics;
        if (Array.isArray(savedTopics) && savedTopics.length) setForm((current) => ({ ...current, topics: savedTopics.join(', ') }));
      } catch (loadError) {
  ...
  // Chỉ mở bước đã có dữ liệu: tránh nhảy vào bài đánh giá/lộ trình trống.
  const stepAvailable = (id) => {
    if (id === 'goal') return true;
    if (id === 'quiz') return questions.length > 0;
    if (id === 'result') return Boolean(assessment);
    return Boolean(roadmap);
  };

  return <div className="tutor-panel">
    {error ? <p className="tutor-error tutor-panel-error">{error}</p> : null}
    <nav className="tutor-steps" aria-label="Các bước lộ trình học">{STEPS.map(([id, label]) => <button type="button" key={id} className={step === id ? 'active' : ''} onClick={() => setStep(id)} disabled={busy !== '' || !stepAvailable(id)}>{label}</button>)}</nav>
